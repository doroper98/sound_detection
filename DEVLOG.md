...
- 최종 소프트웨어 검증(2026-09-24): 코드 `a7e90da`의 [전체 Native iOS 35947609442](https://github.com/doroper98/sound_detection/actions/runs/35947609442)는 Swift 90/90, UI 34/34, Release iPhoneOS 빌드·앱 저장 파일 재생·ZIP 검사·IPA 포장 PASS. [Focused 35947606032](https://github.com/doroper98/sound_detection/actions/runs/35947606032)는 UI 10/10 PASS. 앱이 실제 작성한 합성 세션은 전체 실행에서 중지 21구간/백그라운드 24구간, focused에서 각각 25구간을 Mac CLI로 재생해 불일치 0이었다. 공유 ZIP의 CRC·파일 SHA-256·manifest schema·WAV를 독립 확인했다. 완성 파일을 다시 연 검사이며 실기기 수음 검증은 아니다.
- 전달 검증: focused Release IPA의 CRC·arm64/iPhoneOS·빌드 12·DEBUG 합성 입력 제외를 확인하고 REC/파형/열지도 글씨의 합성 화면을 직접 검토했다. [웹 Verify 35947609634](https://github.com/doroper98/sound_detection/actions/runs/35947609634)의 단위 40/40·E2E 16/16, [재생 계약 35947609481](https://github.com/doroper98/sound_detection/actions/runs/35947609481), Mac/Windows 각 호스트 자체 round-trip PASS. 로컬 gate PASS. [최종 증거](docs/reports/2026-09-24-native-build12-verification.json).
- 앱 ZIP의 Windows 교차 검사: 사용자 PC에서 Swift SDK가 PATH에 없는 portable CLI로 실제 앱이 공유한 합성 ZIP 25구간을 재생했다. 시각 대응은 25/25이고 유효 대역 방향·레벨·판정은 ±1e-6 이내지만, 약 −327 dBFS 무음 대역의 레벨과 directional/omni 비율에 차이가 있어 strict 25/25 불일치(exit 1)다. [필드별 증거](docs/reports/2026-09-24-research-app-windows-difference.json). 이전 CLI 합성 파일의 dB 차이와 구분하며 비교기/운영 기준을 완화하지 않았다. 다음 측정으로 확정 가능한가 **N(같은 합성 ZIP으로 재현 가능)**. 필요한 데이터: synthetic-shared-CF38A7B0-AD85-4EE0-B6C6-98EF6153A51C.zip, Apple/Windows replay.json, 필드별 비교 JSON. 원음 ZIP은 Git에 넣지 않았다.
- 저장소 반영: [PR #9](https://github.com/doroper98/sound_detection/pull/9)를 `047c3d2`로 main 병합했다. 실제 원음 한 파일의 전달·재분석 확인이 다음 단계이고, 실제 최소 10세션/전체 90세션·축 검증·오차 기준선·추정 개선·기존 보정 폐기는 남아 있다. 열지도는 새 관측마다 방향·색·진하기·주파수를 갱신하며 유효하지 않은 관측은 보류한다. 원음 저장이 실기기 정확도 개선을 입증하지는 않는다.
...
- 실측 근거: 사용자 빌드 10 JSON과 화면 3장. FOA 4채널 ACN/SN3D/48 kHz와 별도 Stereo 각각 4,824버퍼, FOA 1,299구간 중 카메라 연결 1,189, 이동 중 거부 110. [선택 필드 및 분석](docs/reports/2026-09-24-iphone17pro-build10-flicker-analysis.json)은 원본 전체의 대체물이 아니다. 카메라 병행 수음은 확인됐지만 축 대응·물리 정확도는 미검증.
- 확인한 실패: 기존 약 85 ms 구간 하나의 통과/탈락을 곧바로 열섬 표시/삭제에 사용했다. 마지막 160–600 Hz 구간은 coherence 0.746이지만 directional/omni energy 3.103(상한 1.8), energyBalance 0.641(하한 0.65)로 거부됐다. 나머지 두 대역도 거부됐다. 모든 거부를 “여러 방향”으로 설명한 문구는 근거가 부족했다. 음량 부족으로 단정하거나 기준을 낮추는 수정은 하지 않았다.
- 미확정 원인: 큰 에너지 비율이 실제 반사/음원 조합/기기 처리 중 무엇 때문인지는 현재 JSON으로 분리할 수 없다. Apple의 ACN/SN3D 레이아웃을 확인했으므로 비율이 약 3이라는 이유로 N3D로 간주해 임의 보정하지 않는다. 표준 SN3D의 단일음+역위상 반사만으로도 비율 >3이 되는 합성 반례를 추가했다. 이 반례가 실제 방의 반사를 입증하는 것은 아니다.
- 표시 수정: 같은 대역의 가까운 세계 방향(18° 이내)을 3회 연속 확인한 뒤 표시한다. 확정 뒤 짧은 불확실 구간은 최대 0.28초까지만 흐리게 표시하고 “직전 관측”을 붙인다. 모든 결과는 0.30초에 만료하고 무음·포화·PCM/카메라 문제·중지 때 즉시 제거한다. 큰 방향 변화는 새 확인이 필요하다. 음향 품질 기준은 그대로다.
- 기록 수정: 기존 시작/종료 UTC 외에 FOA 구간 중간 시각의 UTC 밀리초·첫 구간부터 경과 시간·음향/카메라/표시 판정·대역별 모든 탈락 조건을 기록한다. 최근 240개만 보존하고 제외 수·전체 처리 누계를 남긴다. 화면이 실제 그려진 시각이나 전체 녹화 기록은 아니다. 내보내기 시각·세션 접두·내보내기 UUID로 현재/이전 공유 파일명을 매번 다르게 만든다. 과거 측정 시간은 유지한다.
- UI 수정: 기본 FOA 화면의 큰 중앙 상태 카드·하단 레벨/시간차·반복 설명·범례·FPS·입력 선택을 상세로 옮겼다. 상단 상태, 작은 좌우 파형, 시작/중지 버튼을 유지한다. FOA/기존 열지도 주파수·dBFS 라벨은 카드 배경을 없애고 글씨 그림자만 사용한다.
...
- 중간 검증: `fe349e7` 전체 UI 29/30 및 `0681078` focused UI 5/6. 두 실행 모두 이전 진단 펼치기 뒤 `savedReportShare` 탐색 한 곳에서 실패했다. 계산 78개와 FOA 표시/무음 검사는 통과했다. DisclosureGroup 전체에 부여한 접근성 ID가 하위 버튼과 겹칠 수 있는 구조를 없애고, 별도 펼침 버튼과 기록별 공유 버튼으로 분리했다. 펼침 값·기록 버튼·실제 공유창까지 회귀 검사를 보강한다. 수음·방향 계산 수식은 이 수정에서 변경하지 않았다.
- 2026-09-24 전달 확인: 최종 앱 코드 `752fdad`의 [전체 Mac CI](https://github.com/doroper98/sound_detection/actions/runs/35868716850)는 Swift 78/78·UI 30/30·Release IPA PASS. [focused CI](https://github.com/doroper98/sound_detection/actions/runs/35868713306)도 6/6 PASS. 이전 기록 공유창까지 통과했으며 [웹 CI](https://github.com/doroper98/sound_detection/actions/runs/35868716786)는 단위 40/40·E2E 16/16 PASS. 로컬 gate PASS. Xcode 26.0.1 / SDK 26.0으로 빌드했다.
- 전달 IPA는 focused 실행의 소스 `752fdad135f8259be12eba4cce1aebe08e1afdba`, 658,922바이트, SHA-256 `584df7c75d08bda0c5f0524fbcb6b949df8313300759108f5f249be53566c229`. 전체 실행의 PR 검증 merge `e7bbc2d18b82dc02c16e28bc4c3141edc82b86aa`와 소스 트리가 같고, 두 IPA를 풀었을 때 모든 파일 바이트가 같음을 확인했다. ZIP CRC, arm64 iPhoneOS, 빌드 10, 개인 프로비저닝 없음, Release 합성 플래그 제외를 확인했다. 실제 시뮬레이터 캡처의 열섬·주파수·좌우 파형·상하단 및 이전 보정 버튼 제거를 직접 검토했다. 실기기 설치·FOA 수신·축 대응·반사 환경 정확도는 미검증이다. [전체 기록](docs/reports/2026-09-24-native-build10-verification.json).
