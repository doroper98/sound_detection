...
**iPhone 네이티브 입력:** 빌드 10은 iOS 26 공간 오디오(FOA) 4채널을 분석해 후면 카메라에 방향 열지도와 주파수를 표시하는 실험 경로를 추가했습니다. 별도 스테레오 좌우 파형과 최근 진단 5개 보관도 제공합니다. 6단계 크기 보정은 새 경로에서 사용하지 않습니다. 실제 기기의 FOA 수신·카메라 축 대응·음향 정확도는 미검증이며 거리는 표시하지 않습니다. [현재 사용·검증 순서](docs_canonical/NATIVE_FIELD_VALIDATION.md) · [프로젝트와 설치 기반 안내](native/ios/README.md)
