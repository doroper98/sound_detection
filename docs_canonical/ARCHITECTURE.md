...
오디오 알림은 이유와 실제 입력 상태로 판정한다. category/config 알림에 대해 내장 stereo/portrait/실제 2채널·기존 샘플률을 재검사하고 엔진이 실행 중이면 유지한다. 초기 엔진 재시작은 첫 PCM 전 2초 이내·최대 2회만 허용하며 실제 장치 변경·인터럽트 시작·서비스 손실은 해제한다. 해당 엔진의 알림만 처리하고 notification 내부 스레드에서 엔진을 파괴하지 않는다. schemaVersion 3은 최근 32개 알림 종류/원인 코드/판정과 입력 통계를 추가하며 기기 식별자는 포함하지 않는다.
