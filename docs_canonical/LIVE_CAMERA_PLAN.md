...
## v0.4.0 진행 결과 (2026-09-22)

Safari 원본도 확보했다: [보고서](../docs/reports/2026-09-22-iphone17pro-safari-diagnostics.json). 카메라 켠 상태, 4/2/기본 요청 모두 PCM 2채널 중 채널 1만 활성으로 Chrome 결과와 같다. UA는 `iPhone OS 18_7`, `Version/27.0`을 그대로 보존하며 실제 OS 버전을 확정하지 않는다. 아래 v0.3.0 첫 결과 기록의 ‘Safari 미확보’는 당시 상태다.

이 입력에서도 가능한 `/listen` 연속 주파수·음량 분석을 구현했다. 카메라 시작 → 분석 시작 → 채널/대역 선택 → 분석 JSON 저장 순서다. 공간 열점은 없으며 실제 위치 추정에는 여전히 독립 다채널과 교정이 필요하다. 이 기능의 자동 검증과 실제 iPhone 재검증은 구분한다.
