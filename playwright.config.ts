...
export default defineConfig({
  ...
  use: { baseURL: process.env.PLAYWRIGHT_BASE_URL || 'http://127.0.0.1:5173', browserName: webkit ? 'webkit' : 'chromium', viewport: { width: 1440, height: 900 }, launchOptions: webkit ? {} : { args: ['--use-gl=angle', '--use-angle=swiftshader', '--enable-unsafe-swiftshader'] }, screenshot: 'only-on-failure', trace: 'retain-on-failure' },
