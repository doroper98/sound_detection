...
export default function App() {
  ...
  return <div className="app-shell" data-mobile-view={mobileView} data-sheet={mobileSheet} data-controls={controlTab} data-settings={settingsTab}>
    ...
    <div className="main-shell">
      <header className="topbar"><div className="wordmark">soundfield<span>LAB</span></div><div className="breadcrumb">워크스페이스 <span>/</span> <strong>음향 위치 실험실</strong></div><div className="header-right"><span className="version-tag">v{__APP_VERSION__}</span><a className="text-button" href="/listen" aria-label="실제 카메라·소리 분석" title="실제 카메라·소리 분석"><Mic size={16} /><span>실제 소리 분석</span></a><button className="text-button" aria-label="사용 가이드" onClick={() => setModal('guide')}><CircleHelp size={16} /><span>사용 가이드</span><ArrowUpRight size={14} /></button></div></header>
      <main>
