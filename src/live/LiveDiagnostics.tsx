...
export default function LiveDiagnostics() {
  ...
  return <div className="live-page">
    <header className="live-header"><a href="/" aria-label="시뮬레이터로 돌아가기"><ArrowLeft size={17} /><span>soundfield</span></a><span className="live-version">실제 입력 진단 · v{__APP_VERSION__}</span></header>
    <div className="live-title"><div><span className="eyebrow">DEVICE CHECK</span><h1>내 아이폰은 몇 채널을 들을까?</h1><p>카메라와 실제 수음 확인 · 음원 위치 추정은 아직 미검증입니다.</p></div><span className="live-target">iPhone 17 Pro · Safari</span></div>
    <div className="live-grid">
      <section className="live-preview" aria-label="실제 카메라 미리보기">
        ...
        <div className="live-meter"><span>{running ? '실시간 PCM' : '마이크 대기'}</span><strong>{levels ? Math.max(...levels.channels.map(c => c.levelDbfs ?? -120)).toFixed(1) : '—'}<small>dBFS</small></strong><p>디지털 음량 · 실제 dB SPL 교정 전</p></div>
        {cameraError && <p className="camera-error" role="alert">{cameraError}</p>}
        <div className="live-camera-controls"><button className="button secondary" disabled={running && cameraState === 'off'} onClick={cameraState === 'off' ? () => void startCamera() : stopCamera}>{cameraState === 'off' ? <Camera size={16} /> : <Square size={15} />}{cameraState === 'off' ? '카메라 시작' : '카메라 중지'}</button></div>
      </section>
      <section className="live-diagnostics" aria-label="마이크 채널 진단">
