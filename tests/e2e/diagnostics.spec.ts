...
async function installMediaFixture(page: Page, mode: 'mono' | 'four' | 'duplicate' | 'ignored' | 'denied' | 'transport') {
  await page.addInitScript(mode => {
    ...
    AudioContext.prototype.createMediaStreamSource = function(stream: MediaStream) {
      ...
      for (let c = 0; c < count; c++) {
        ...
        for (let i = 0; i < samples.length; i++) samples[i] = Math.sin(i * 2 * Math.PI * (mode === 'duplicate' ? 440 : 300 + c * 313) / this.sampleRate) * .1;
      }
    ...
    Object.defineProperty(navigator.mediaDevices, 'getUserMedia', { configurable: true, value: async (constraints: MediaStreamConstraints) => {
      ...
      const count = mode === 'mono' || mode === 'ignored' ? 1 : 4;
